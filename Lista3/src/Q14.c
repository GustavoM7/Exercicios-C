...
minúsculos em maiúsculos e os maiúsculos em minúsculos.
b) int ocorrencia(char *string, char caractere): que recebe uma string e um
caractere, e retorne o número de vezes que esse caractere aparece na string.
c) void remove(char *string, char caractere): que recebe uma string e um
caractere, e apague todas as ocorrências desses caractere na string.
d) void titulo(char *string): que altera a primeira letra de cada uma das palavras da
string deixando-a em maiúsculo.
e) void inverte(char *string): que receba uma string e a retorne escrita de trás pra
frente .
*/
